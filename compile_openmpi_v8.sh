...
# FLAGS PARA CONFIGURAR: 
# Incorporamos _GNU_SOURCE aquí para que se propague correctamente a todos los Makefiles
# sin necesidad de sobrescribir variables en el comando 'make'.
CONF_CFLAGS="-fPIC -Oz"
...
# Ejecutando make sin sobrescribir las variables para respetar la configuración.
make -j"$(nproc)" all install DESTDIR="$DESTDIR" 2>&1 | tee build_install.out
