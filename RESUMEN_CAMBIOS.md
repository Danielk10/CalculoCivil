...
1. **Reestructuración de Binarios**:
   ...
   - Se renombraron para cumplir estrictamente con las políticas de Google: todo binario debe empezar con `lib` y terminar con `.so` sin puntos intermedios en el nombre base. (Ej: `OpenSees` -> `libOpenSees.so`).
